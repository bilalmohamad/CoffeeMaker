...
public class APIRecipeController extends APIController {
     ...
     * @return response to the request
     */
    @PutMapping ( BASE_PATH + "/recipes/{id}" )
    public ResponseEntity updateRecipe ( @PathVariable ( "id" ) final Long id, @RequestBody final Recipe recipe ) {
        final Recipe oldRecipe = (Recipe) Recipe.getById( Recipe.class, id );
        if ( null == oldRecipe ) {
            return new ResponseEntity( errorResponse( "No recipe found for id " + id ), HttpStatus.NOT_FOUND );
        }
        if ( recipe.getPrice() < 0 ) {
            return new ResponseEntity( errorResponse( "Invalid price" ), HttpStatus.NOT_MODIFIED );
        }
        if ( recipe.getCoffee() < 0 ) {
            return new ResponseEntity( errorResponse( "Invalid coffee amount" ), HttpStatus.NOT_MODIFIED );
        }
        if ( recipe.getMilk() < 0 ) {
            return new ResponseEntity( errorResponse( "Invalid milk amount" ), HttpStatus.NOT_MODIFIED );
        }
        if ( recipe.getSugar() < 0 ) {
            return new ResponseEntity( errorResponse( "Invalid sugar amount" ), HttpStatus.NOT_MODIFIED );
        }
        if ( recipe.getChocolate() < 0 ) {
            return new ResponseEntity( errorResponse( "Invalid chocolate amount" ), HttpStatus.NOT_MODIFIED );
        }

        oldRecipe.updateRecipe( recipe );
        oldRecipe.save();

        return new ResponseEntity( successResponse( oldRecipe.getName() + " was edited successfully" ), HttpStatus.OK );
    }
}
